...
If remove = 1 Then
Dim kk As Integer
Do While kk<GetNumberOfParameters
kk=kk+1
If DoesParameterExist(GetParameterName(kk)) Then
	DeleteParameter(kk)
End If
Loop
End If
...
If DoesParameterExist("cst_core_ang") Then
	Exit Sub
End If
...
'HAND1:
	Begin Dialog UserDialog 640,343,"Create 3D Toroidal Coil, rectangular core." ' %GRID:10,7,1,1
		...
		'DropListBox 490,210,140,14,A(),.DropListBox1
		'PushButton 490,238,130,28,"update",.updateButton
	End Dialog
	Dim dlg As UserDialog

	'Debug.Print(.updateButton)
...
BeginHide
	...
	SetParameterDescription  ( "cst_h_gnd",  "Heigth towards GND-plane"  )
